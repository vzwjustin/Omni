...
from .enhanced_models import CacheEntry, CacheMetadata
...
class ContextCache:
    ...
    async def get(
        ...
        if entry.is_expired:
            if allow_stale and self._enable_stale_fallback:
                # Track stale hit
                await self._track_cache_hit_async(cache_type, is_stale=True, age_seconds=entry.age.total_seconds())
        ...
        # Track hit
        await self._track_cache_hit_async(cache_type, is_stale=False, age_seconds=entry.age.total_seconds())
    ...
    def _process_invalidation_queue(self) -> None:
        ...
        while self._invalidation_queue:
            ...
            try:
                ...
                logger.warning(
                    "invalidation_queue_processing_failed",
                    workspace=workspace_path,
                    error=str(e)
                )
    ...
    def _stop_watching_workspace(self, workspace_path: str) -> None:
        ...
        if workspace_path in self._observers:
            try:
                ...
                logger.warning(
                    "workspace_watching_stop_failed",
                    workspace=workspace_path,
                    error=str(e)
                )
    ...
    async def _track_cache_hit_async(self, cache_type: str, is_stale: bool, age_seconds: float) -> None:
        """
        ...
        async with self._stats_lock:
            if is_stale:
                self._cache_stats["stale_hits"][cache_type] = self._cache_stats["stale_hits"].get(cache_type, 0) + 1
            else:
                self._cache_stats["hits"][cache_type] = self._cache_stats["hits"].get(cache_type, 0) + 1
            ...
            self._cache_stats["tokens_saved"][cache_type] = self._cache_stats["tokens_saved"].get(cache_type, 0) + tokens_saved
    ...
    async def _track_cache_miss_async(self, cache_type: str) -> None:
        ...
        """
        async with self._stats_lock:
            self._cache_stats["misses"][cache_type] = self._cache_stats["misses"].get(cache_type, 0) + 1
    ...
    def _track_invalidation(self, reason: str, count: int) -> None:
        ...
        """
        self._cache_stats["invalidations"][reason] = self._cache_stats["invalidations"].get(reason, 0) + count
