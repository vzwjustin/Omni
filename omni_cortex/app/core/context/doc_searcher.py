...
from .enhanced_models import SourceAttribution, EnhancedDocumentationContext
            ...
            if hasattr(metadata, 'grounding_supports'):
                for support in metadata.grounding_supports:
                    if hasattr(support, 'source') and hasattr(support.source, 'uri'):
                        ...
                        attributions.append(SourceAttribution(
                            url=url,
                            title=support.source.title if hasattr(support.source, 'title') else "Documentation",
                            domain=domain,
                            authority_score=authority_score,
                            is_official=is_official,
                            grounding_metadata={
...
Focus on actionable, technical content."""
            ...
            if text and len(text) > 50:
                ...
                if attributions:
                    ...
                    for i, attribution in enumerate(attributions[:5]):  # Limit to top 5
                        docs.append(EnhancedDocumentationContext(
                            source=attribution.url,
                            title=attribution.title,
                            snippet=text[:CONTENT.SNIPPET_MAX] if i == 0 else text[:CONTENT.SNIPPET_EXTENDED],
            ...
            self.logger.debug("web_search_with_attribution_complete", docs_found=len(docs), attributions=len(attributions))
            return docs
            
        except (LLMError, ProviderNotConfiguredError):
            raise
        except Exception as e:
            self.logger.warning("doc_search_with_attribution_failed", error=str(e), error_type=type(e).__name__)
            return []
    ...
    async def _merge_web_and_local_results(
        ...
        for local in local_results:
            enhanced_local.append(EnhancedDocumentationContext(
                source=local.source,
                title=local.title,
        ...
        self.logger.debug("merged_results", 
    ...
    def _prioritize_by_authority(
        self,
        results: List[EnhancedDocumentationContext]
    ) -> List[EnhancedDocumentationContext]:
        """
        ...
        self.logger.debug("prioritized_by_authority",
    ...
    async def search_with_fallback(
        ...
        if web_search_failed and prioritized:
            ...
            if prioritized[0].snippet:
                prioritized[0].snippet = f"[Note: Web search unavailable, showing local results only]\n\n{prioritized[0].snippet}"
        
        self.logger.info("search_with_fallback_complete",
