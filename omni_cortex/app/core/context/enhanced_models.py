...
class ProgressEvent:
    ...
    estimated_completion: float | None = None  # seconds remaining
...
class QualityMetrics:
    ...
    relevance_distribution: list[float] = field(
        default_factory=list
    )  # Relevance scores of all items
    relevance_metrics: RelevanceMetrics | None = None  # Relevance tracking metrics
...
class EnhancedFileContext:
    ...
    line_count: int = 0
    size_kb: float = 0
    repository: str | None = None  # Repository name if multi-repo
    last_modified: datetime | None = None
    git_blame_info: dict[str, Any] | None = None
...
class EnhancedStructuredContext:
    ...
    chain_suggestion: list[str] | None = None
    ...
    token_budget_usage: TokenBudgetUsage | None = None
...
**Reason**: {self.framework_reason}""")
        ...
        if self.token_budget_usage:
            ...
            utilization_bar = "█" * int(usage.utilization_percentage / 20) + "░" * (
                5 - int(usage.utilization_percentage / 20)
            )
            budget_lines.append(
                f"**Usage**: [{utilization_bar}] {usage.utilization_percentage:.1f}% ({usage.actual_usage:,}/{usage.allocated_budget:,} tokens)"
            )
            if usage.optimization_applied:
                budget_lines.append(
                    "**Optimizations Applied**: " + ", ".join(usage.optimization_details)
                )
            sections.append("\n".join(budget_lines))
    ...
    def to_detailed_json(self) -> dict[str, Any]:
        ...
        return {
            ...
            "token_budget": {
                "allocated": self.token_budget_usage.allocated_budget
                if self.token_budget_usage
                else self.token_budget,
                "used": self.token_budget_usage.actual_usage
                if self.token_budget_usage
                else self.actual_tokens,
                "utilization": self.token_budget_usage.utilization_percentage
                if self.token_budget_usage
                else 0,
            },
