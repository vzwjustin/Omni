...
from .enhanced_models import CrossRepoDependency, EnhancedFileContext, RepoInfo
...
class MultiRepoFileDiscoverer(FileDiscoverer):
    ...
    async def discover_multi_repo(
        self, query: str, workspace_path: str, max_files: int = 15
        ...
        if max_repos and len(repos) > max_repos:
            logger.warning("multi_repo_limit_applied", detected=len(repos), max_allowed=max_repos)
    ...
    def generate_repository_warnings(self, repos: list[RepoInfo]) -> list[str]:
        """
        ...
        for repo in repos:
            if not repo.is_accessible:
                ...
                warnings.append(f"⚠️ Repository '{repo.name}' has limited read permissions")
            elif not repo.last_commit:
                warnings.append(f"⚠️ Repository '{repo.name}' may not be a valid git repository")
    ...
    def _create_repo_info(self, repo_path: str, workspace_path: str) -> RepoInfo | None:
        """
        ...
        try:
            ...
            logger.warning("repo_info_creation_failed", repo_path=repo_path, error=str(e))
            return RepoInfo(
                path=repo_path,
    ...
    def _load_gitignore_patterns(self, repo_path: str) -> list[str]:
        ...
        if gitignore_path.exists():
            try:
                ...
                logger.warning("gitignore_load_failed", repo_path=repo_path, error=str(e))

        return patterns

    async def _analyze_repositories_parallel(
        self, query: str, repos: list[RepoInfo], max_files_per_repo: int
        ...
        try:
            ...
            logger.error("repo_analysis_timeout", repos=[r.name for r in accessible_repos])
    ...
    async def _analyze_repository(
        self, repo: RepoInfo, query: str, max_files: int
    ) -> list[FileContext]:
        """
        ...
        try:
            ...
            files = await self.discover(query=query, workspace_path=repo.path, max_files=max_files)

            # Filter files based on .gitignore patterns
            if repo.ignore_patterns:
                files = self._filter_by_gitignore(files, repo)

            logger.info("repo_analysis_complete", repo=repo.name, files_found=len(files))
    ...
    def _filter_by_gitignore(self, files: list[FileContext], repo: RepoInfo) -> list[FileContext]:
        """
    ...
    async def _follow_cross_repo_dependencies(
        self, repos: list[RepoInfo], repo_results: dict[RepoInfo, list[FileContext]]
            ...
            for file_ctx in files:
                ...
                try:
                    ...
                    file_deps = self._detect_cross_repo_references(
                        content, file_ctx.path, repo, repo_by_name
                    )
        ...
        logger.info("cross_repo_dependencies_detected", count=len(dependencies))

        return dependencies

    def _read_file_safe(self, file_path: Path, max_size_kb: int = 500) -> str | None:
        """
    ...
    def build_dependency_graph(
        self, dependencies: list[CrossRepoDependency]
    ...
    def get_transitive_dependencies(
        ...
        if repo_name in graph:
            for target_repo in graph[repo_name]:
                ...
                transitive = self.get_transitive_dependencies(target_repo, graph, visited.copy())
