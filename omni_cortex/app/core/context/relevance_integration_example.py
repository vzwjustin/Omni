...
from app.core.context_gateway import StructuredContext, get_context_gateway
...
async def prepare_context_with_tracking(
    query: str, workspace_path: str | None = None, optimize_scores: bool = True
    ...
    context = await gateway.prepare_context(query=query, workspace_path=workspace_path)
    ...
    tracker.record_context_preparation(
        ...
        query=query,
        files=[
            {"path": f.path, "relevance_score": f.relevance_score} for f in context.relevant_files
        ],
        documentation=[
            {"source": d.source, "relevance_score": d.relevance_score}
            for d in context.documentation
        ],
        code_search=[{"search_type": c.search_type, "query": c.query} for c in context.code_search],
    ...
    record_relevance_session(
        task_type=context.task_type,
        elements_included=len(context.relevant_files)
        + len(context.documentation)
        + len(context.code_search),
...
async def record_solution_with_tracking(session_id: str, solution_text: str) -> dict:
    """
...
async def get_relevance_insights(task_type: str | None = None) -> dict:
    """
...
def _generate_recommendations(feedback: dict, stats: dict) -> list[str]:
    ...
    if avg_rate < 0.5:
        ...
        recommendations.append(
            f"Excellent file usage rate of {avg_rate:.2%}. Context preparation is highly effective"
        )
...
async def optimize_future_context(files: list[dict], task_type: str | None = None) -> list[dict]:
    """
    ...
    # Count adjustments
    adjustments = {"boosted": 0, "reduced": 0, "unchanged": 0, "no_history": 0}
    ...
    for adjustment_type, count in adjustments.items():
        if count > 0:
            record_relevance_optimization(
                task_type=task_type or "unknown", adjustment_type=adjustment_type, count=count
            )
...
async def example_workflow():
    ...
    context, session_id = await prepare_context_with_tracking(
        query=query, workspace_path="/path/to/project"
    )
