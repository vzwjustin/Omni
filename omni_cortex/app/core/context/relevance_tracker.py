...
class ElementUsage:
    ...
    last_included: Optional[datetime] = None
...
class ContextUsageSession:
    ...
    solution_text: Optional[str] = None  # Claude's solution for analysis
...
class RelevanceTracker:
    ...
    def record_context_preparation(
        ...
        for code_ctx in code_search:
            element_id = f"code_search:{code_ctx.get('search_type', '')}:{code_ctx.get('query', '')}"
            session.included_elements.add(element_id)
            
            if element_id not in self._element_usage:
                self._element_usage[element_id] = ElementUsage(
                    element_id=element_id,
                    element_type="code_search"
                )
    ...
    def record_solution_usage(
        ...
        logger.info(
            ...
            usage_rate=len(session.used_elements) / len(session.included_elements) if session.included_elements else 0
        )
    ...
    def get_relevance_feedback(
        ...
        feedback = {
            "high_value_files": [
                ...
                for e in files[:top_n] if e.usage_rate > 0.5
            ],
            "low_value_files": [
                {
                    "path": e.element_id.replace("file:", ""),
                ...
                for e in files[-top_n:] if e.usage_rate < 0.2 and e.times_included >= 3
            ],
            "high_value_docs": [
                {
                ...
                for e in docs[:top_n] if e.usage_rate > 0.5
            ],
            "low_value_docs": [
                {
                    "source": e.element_id.replace("doc:", ""),
                ...
                for e in docs[-top_n:] if e.usage_rate < 0.2 and e.times_included >= 3
            ],
            "overall_stats": {
                "total_elements": len(elements),
                "avg_usage_rate": sum(e.usage_rate for e in elements) / len(elements) if elements else 0,
                "high_value_count": len([e for e in elements if e.usage_rate > 0.5]),
                "low_value_count": len([e for e in elements if e.usage_rate < 0.2 and e.times_included >= 3])
            }
        }
    ...
    def get_summary_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "low_value_files": len([f for f in files if f.usage_rate < 0.2 and f.times_included >= 3]),
...
_relevance_tracker: Optional[RelevanceTracker] = None
