...
class TokenBudgetManager:
    ...
    def allocate_budget(
        ...
        allocation_percentages = {
            k: v / total_percentage
            for k, v in allocation_percentages.items()
        }
    ...
    def create_usage_report(
        ...
        """
        utilization = (actual_usage / allocation.total_budget * 100) if allocation.total_budget > 0 else 0
...
class GeminiContentRanker:
    ...
    async def rank_documentation(
        ...
        if not self._model:
            ...
            return sorted_docs[:max_docs], ["Fallback: sorted by relevance score (Gemini unavailable)"]
...
Ranking (indices only, comma-separated):"""
            ...
            try:
                ...
                optimizations.append(f"Gemini ranked {len(docs)} docs, selected top {len(ranked_docs)}")
                
                logger.info(
                    "gemini_docs_ranked",
                    total=len(docs),
                    ranked=len(ranked_docs),
                    query=query[:50]
                )
                ...
                sorted_docs = sorted(docs, key=lambda d: d.relevance_score, reverse=True)
                return sorted_docs[:max_docs], ["Fallback: Gemini ranking parse failed, using relevance scores"]
        
        except Exception as e:
            logger.warning("gemini_ranking_failed", error=str(e))
            # Fallback to relevance score sorting
            sorted_docs = sorted(docs, key=lambda d: d.relevance_score, reverse=True)
            return sorted_docs[:max_docs], [f"Fallback: Gemini ranking failed ({str(e)}), using relevance scores"]
...
Summary:"""
            ...
            optimizations.append(f"Gemini summarized {len(code_search_results)} code search results into {len(summary)} chars")
            ...
            # Fallback to truncation
            combined = "\n".join([str(r.results)[:200] for r in code_search_results[:3]])
            return combined[:max_length], [f"Fallback: Gemini summarization failed ({str(e)}), using truncation"]
    ...
    async def optimize_content_for_budget(
        ...
        filtered_files, file_opts = await self.filter_low_value_content(query, files, relevance_threshold=0.3)
        all_optimizations.extend(file_opts)
        
        # Summarize code patterns
        code_summary, code_opts = await self.summarize_code_patterns(query, code_search_results, max_length=code_budget * 4)
...
_gemini_ranker: Optional[GeminiContentRanker] = None
