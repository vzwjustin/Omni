...
MCP_REQUESTS = Counter(
    "omni_cortex_mcp_requests_total",
...
RATE_LIMIT_REJECTIONS = Counter(
    "omni_cortex_rate_limit_rejections_total",
    "Total rate limit rejections",
    ["tool", "category"]
)

RATE_LIMIT_TOKENS = Gauge(
    "omni_cortex_rate_limit_tokens",
...
MEMORY_THREADS = Gauge(
    "omni_cortex_memory_threads_active",
...
CACHE_MISSES = Counter(
    "omni_cortex_cache_misses_total",
    "Cache misses",
    ["cache_type"]
)
...
CONTEXT_CACHE_HITS = Counter(
    ...
    ["cache_type", "stale"]  # cache_type: query_analysis, file_discovery, documentation; stale: true/false
)

CONTEXT_CACHE_MISSES = Counter(
    "omni_cortex_context_cache_misses_total",
    "Context cache misses by cache type",
    ["cache_type"]
)
...
CONTEXT_CACHE_SIZE = Gauge(
    "omni_cortex_context_cache_size_bytes",
    "Current context cache size in bytes"
)
...
RAG_SEARCHES = Counter(
    "omni_cortex_rag_searches_total",
    "Total RAG searches",
    ["collection"]
)
...
SYSTEM_INFO = Info(
    "omni_cortex",
    "Omni-Cortex system information"
)
...
def record_relevance_optimization(
    task_type: str,
    adjustment_type: str,
    count: int = 1
) -> None:
