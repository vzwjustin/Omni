...
class RateLimiter:
    ...
    def __init__(self, config: RateLimitConfig | None = None):
        ...
        self._buckets: dict[str, TokenBucket] = {
            "llm": TokenBucket(
                capacity=self.config.llm_tools_rpm, refill_rate=self.config.llm_tools_rpm / 60.0
            ),
            "search": TokenBucket(
            ...
            "global": TokenBucket(
                capacity=self.config.global_rpm, refill_rate=self.config.global_rpm / 60.0
            ),
        }
    ...
    async def check_rate_limit(self, tool_name: str) -> tuple[bool, str]:
        ...
        async with self._lock:
            ...
            if not category_bucket.try_acquire():
                ...
                global_bucket.tokens = min(global_bucket.capacity, global_bucket.tokens + 1)
    ...
    def validate_input_size(self, arguments: dict[str, Any], tool_name: str) -> tuple[bool, str]:
        ...
        if total_size > self.config.max_input_size:
            ...
            return (
                False,
                f"Input size ({total_size} bytes) exceeds limit ({self.config.max_input_size} bytes)",
            )
