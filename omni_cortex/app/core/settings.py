...
class OmniCortexSettings(BaseSettings):
    ...
    routing_cache_ttl_seconds: int = Field(
        default=300, ge=60, le=3600, alias="ROUTING_CACHE_TTL_SECONDS"
    )
    ...
    checkpoint_path: Path = Field(
        default=Path("/app/data/checkpoints.sqlite"), alias="CHECKPOINT_PATH"
    )
    watch_root: Path | None = Field(default=None, alias="WATCH_ROOT")
    ...
    cache_query_analysis_ttl: int = Field(
        default=3600, ge=60, le=86400, alias="CACHE_QUERY_ANALYSIS_TTL"
    )
    cache_file_discovery_ttl: int = Field(
        default=1800, ge=60, le=86400, alias="CACHE_FILE_DISCOVERY_TTL"
    )
    cache_documentation_ttl: int = Field(
        default=86400, ge=3600, le=604800, alias="CACHE_DOCUMENTATION_TTL"
    )
    ...
    circuit_breaker_failure_threshold: int = Field(
        default=5, ge=1, le=20, alias="CIRCUIT_BREAKER_FAILURE_THRESHOLD"
    )
    circuit_breaker_recovery_timeout: int = Field(
        default=60, ge=10, le=600, alias="CIRCUIT_BREAKER_RECOVERY_TIMEOUT"
    )
    circuit_breaker_half_open_timeout: int = Field(
        default=30, ge=5, le=300, alias="CIRCUIT_BREAKER_HALF_OPEN_TIMEOUT"
    )
    ...
    streaming_progress_interval: float = Field(
        default=0.5, ge=0.1, le=5.0, alias="STREAMING_PROGRESS_INTERVAL"
    )
    ...
    multi_repo_max_repositories: int = Field(
        default=10, ge=1, le=50, alias="MULTI_REPO_MAX_REPOSITORIES"
    )
    multi_repo_max_concurrent: int = Field(
        default=5, ge=1, le=20, alias="MULTI_REPO_MAX_CONCURRENT"
    )
    multi_repo_analysis_timeout: float = Field(
        default=30.0, ge=5.0, le=300.0, alias="MULTI_REPO_ANALYSIS_TIMEOUT"
    )
    enable_cross_repo_dependencies: bool = Field(
        default=True, alias="ENABLE_CROSS_REPO_DEPENDENCIES"
    )

    # Token budget management
    enable_dynamic_token_budget: bool = Field(default=True, alias="ENABLE_DYNAMIC_TOKEN_BUDGET")
    token_budget_low_complexity: int = Field(
        default=30000, ge=10000, le=200000, alias="TOKEN_BUDGET_LOW_COMPLEXITY"
    )
    token_budget_medium_complexity: int = Field(
        default=50000, ge=20000, le=200000, alias="TOKEN_BUDGET_MEDIUM_COMPLEXITY"
    )
    token_budget_high_complexity: int = Field(
        default=80000, ge=40000, le=200000, alias="TOKEN_BUDGET_HIGH_COMPLEXITY"
    )
    token_budget_very_high_complexity: int = Field(
        default=120000, ge=60000, le=300000, alias="TOKEN_BUDGET_VERY_HIGH_COMPLEXITY"
    )
    ...
    enable_documentation_prioritization: bool = Field(
        default=True, alias="ENABLE_DOCUMENTATION_PRIORITIZATION"
    )
    documentation_authority_threshold: float = Field(
        default=0.7, ge=0.0, le=1.0, alias="DOCUMENTATION_AUTHORITY_THRESHOLD"
    )
    ...
    thinking_mode_complexity_threshold: str = Field(
        default="medium", alias="THINKING_MODE_COMPLEXITY_THRESHOLD"
    )
    thinking_mode_token_threshold: int = Field(
        default=20000, ge=5000, le=100000, alias="THINKING_MODE_TOKEN_THRESHOLD"
    )
    ...
    llmlingua_compression_rate: float = Field(
        default=0.5, ge=0.1, le=0.9, alias="LLMLINGUA_COMPRESSION_RATE"
    )
    ...
    compression_min_tokens: int = Field(
        default=5000, ge=1000, le=50000, alias="COMPRESSION_MIN_TOKENS"
    )
    ...
    def validate_thinking_mode_threshold(cls, v: str) -> str:
        ...
        if v.lower() not in valid_thresholds:
            raise ValueError(
                f"thinking_mode_complexity_threshold must be one of {valid_thresholds}"
            )
        return v.lower()
...
_settings: OmniCortexSettings | None = None
