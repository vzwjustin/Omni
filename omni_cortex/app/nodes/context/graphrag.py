...
async def _extract_relationships(
    entities: list[Entity], query: str, code_context: str, state: GraphState
) -> list[Relationship]:
...
async def _query_graph(
    entities: list[Entity], relationships: list[Relationship], query: str, state: GraphState
) -> list[int]:
...
async def _traverse_graph_paths(
    start_ids: list[int],
...
async def graphrag_node(state: GraphState) -> GraphState:
    ...
    # Synthesize
    answer = await _synthesize_from_graph(
        paths, entities, relationships, query, code_context, state
    )
