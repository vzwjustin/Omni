...
async def _score_relevance(
    hypo_doc: str, query: str, code_context: str, state: GraphState
) -> float:
...
async def _synthesize_actual_answer(
    hypo_docs: list[HypotheticalDoc], query: str, code_context: str, state: GraphState
) -> str:
...
async def hyde_node(state: GraphState) -> GraphState:
    ...
    add_reasoning_step(
        state=state, framework="hyde", thought="Expanded with rich details", action="expand"
    )
    ...
    for i, var in enumerate(variations, 1):
        ...
        hypo_docs.append(HypotheticalDoc(doc_num=i, content=var, relevance_score=score))
