...
async def _generate_thoughts(
    query: str, code_context: str, buffer: list[BufferEntry], state: GraphState
) -> list[str]:
...
async def _distill_insight(thought: str, query: str, state: GraphState) -> str:
...
async def _score_relevance(insight: str, query: str, state: GraphState) -> float:
...
async def _synthesize_from_buffer(
    buffer: list[BufferEntry], query: str, code_context: str, state: GraphState
) -> str:
    ...
    insights = "\n".join(
        [
            f"- {entry.distilled_insight} (relevance: {entry.relevance_score:.2f})"
...
"""
    ...
    state["confidence_score"] = (
        sum(e.relevance_score for e in buffer) / len(buffer) if buffer else 0.5
    )
