...
async def _verify_chain(
    steps: list[ReasoningStep], query: str, state: GraphState
) -> tuple[bool, str]:
...
async def _synthesize_answer(
    steps: list[ReasoningStep], query: str, code_context: str, state: GraphState
) -> str:
    """Synthesize final answer from reasoning chain."""

    chain = "\n\n".join(
        [f"**Step {s.step_num}**: {s.question}\nConclusion: {s.conclusion}" for s in steps]
    )
...
async def chain_of_thought_node(state: GraphState) -> GraphState:
    ...
    for i, question in enumerate(step_questions, 1):
        step = await _reason_about_step(question, i, reasoning_steps, query, code_context, state)
