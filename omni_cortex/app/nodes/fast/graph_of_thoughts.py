...
async def _identify_dependencies(nodes: list[ThoughtNode], query: str, state: GraphState) -> None:
...
async def _score_nodes(
    nodes: list[ThoughtNode], query: str, code_context: str, state: GraphState
) -> None:
    """Score quality of each thought node."""

    tasks = [
        process_reward_model(step=node.content, context=code_context, goal=query) for node in nodes
    ]
...
async def _synthesize_from_path(
    path: list[ThoughtNode], query: str, code_context: str, state: GraphState
) -> str:
