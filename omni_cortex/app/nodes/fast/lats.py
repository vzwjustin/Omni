...
async def _generate_action_candidates(
    query: str, code_context: str, current_path: list[str], state: GraphState
) -> list[str]:
...
async def _execute_action(action: str, query: str, code_context: str, state: GraphState) -> str:
...
async def _synthesize_solution(
    best_path: list[ActionNode], query: str, code_context: str, state: GraphState
) -> str:
...
async def lats_node(state: GraphState) -> GraphState:
    ...
    for depth in range(MAX_DEPTH):
        ...
        for node in frontier:
            # Generate candidates
            candidates = await _generate_action_candidates(query, code_context, node.path(), state)
