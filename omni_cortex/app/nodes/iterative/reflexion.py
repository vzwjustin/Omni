...
async def _make_attempt(
    query: str, code_context: str, previous_attempts: list[Attempt], state: GraphState
) -> str:
    ...
    if previous_attempts:
        ...
        for i, attempt in enumerate(previous_attempts, 1):
            ...
            lessons_context += "**Lessons learned:**\n"
...
async def _evaluate_attempt(
    solution: str, query: str, code_context: str, attempt_num: int, state: GraphState
...
async def _reflect_on_failure(
    attempt: Attempt, query: str, code_context: str, state: GraphState
...
async def _synthesize_final_solution(
    attempts: list[Attempt], query: str, code_context: str, state: GraphState
) -> str:
...
async def reflexion_node(state: GraphState) -> GraphState:
    ...
    for attempt_num in range(1, MAX_ATTEMPTS + 1):
        ...
        failure_reasons, lessons = await _reflect_on_failure(attempt, query, code_context, state)
    ...
    if attempts:
        ...
        if not final_success and len(attempts) > 1:
            final_solution = await _synthesize_final_solution(attempts, query, code_context, state)
        else:
...
"""
    ...
    logger.info(
        "reflexion_complete", attempts=len(attempts), success=final_success, final_score=final_score
    )
