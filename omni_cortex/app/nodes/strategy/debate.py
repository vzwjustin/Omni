...
async def _generate_positions(query: str, code_context: str, state: GraphState) -> list[Agent]:
...
async def _agent_argue(
    agent: Agent, query: str, code_context: str, other_positions: list[str], state: GraphState
) -> list[str]:
...
async def _agent_rebut(agent: Agent, target_agent: Agent, query: str, state: GraphState) -> str:
...
async def _judge_debate(
    agents: list[Agent], query: str, code_context: str, state: GraphState
) -> tuple[Agent, str]:
...
async def multi_agent_debate_node(state: GraphState) -> GraphState:
    ...
    for round_num in range(NUM_ROUNDS):
        ...
        for agent in agents:
            ...
            agent.arguments = await _agent_argue(agent, query, code_context, other_positions, state)
