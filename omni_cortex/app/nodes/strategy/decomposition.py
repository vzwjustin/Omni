...
async def _decompose_problem(query: str, code_context: str, state: GraphState) -> list[SubProblem]:
...
"""
    ...
    for line in response.split("\n"):
        ...
        if line.startswith("SUBPROBLEM_"):
            if current_desc:
                subproblems.append(
                    SubProblem(
                        id=len(subproblems) + 1, description=current_desc, dependencies=current_deps
                    )
                )
    ...
    if current_desc:
        subproblems.append(
            SubProblem(id=len(subproblems) + 1, description=current_desc, dependencies=current_deps)
        )
...
async def _integrate_solutions(
    subproblems: list[SubProblem], query: str, code_context: str, state: GraphState
) -> str:
