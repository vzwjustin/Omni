...
async def _red_team_attack(solution: str, round_num: int, previous_attacks: list[Attack], query: str, state: GraphState) -> list[Attack]:
    """Red team attacks the solution."""
    
    prev_attacks_text = ""
    if previous_attacks:
        prev_attacks_text = "\n\nPREVIOUS ATTACKS:\n"
        prev_attacks_text += "\n".join(f"- {a.attack_vector}: {a.vulnerability_found}" for a in previous_attacks[-3:])
...
async def _harden_solution(solution: str, attacks: list[Attack], query: str, state: GraphState) -> str:
    """Harden solution against attacks."""
    
    attacks_text = "\n".join([
        f"- {a.attack_vector} ({a.severity}): {a.vulnerability_found}"
        for a in attacks
    ])
...
async def _verify_hardening(hardened: str, original_attacks: list[Attack], query: str, state: GraphState) -> tuple[bool, str]:
