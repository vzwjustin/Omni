...
async def _generate_alternative(query: str, code_context: str, alt_num: int, state: GraphState) -> str:
...
async def _check_consistency(main_answer: AlternativeAnswer, alternatives: list[AlternativeAnswer], state: GraphState) -> tuple[list[str], list[str]]:
...
async def _revise_answer(original: str, consistent: list[str], inconsistent: list[str], query: str, code_context: str, state: GraphState) -> str:
...
async def selfcheckgpt_node(state: GraphState) -> GraphState:
    ...
    consistency_rate = len(consistent) / (len(consistent) + len(inconsistent)) if (len(consistent) + len(inconsistent)) > 0 else 1.0
    ...
    # Revise if needed
    if inconsistent:
        revised = await _revise_answer(initial, consistent, inconsistent, query, code_context, state)
