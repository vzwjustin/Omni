...
Plan steps and what you EXPECT to observe at each step.""",
    ...
    return {"final_answer": final, "metadata": {"framework": "rewoo"}}
...
Provide complete solution.""",
    ...
    return {"final_answer": final, "metadata": {"framework": "lats", "branches": len(branches)}}
...
Resolve conflicts.""",
    ...
    return {"final_answer": synthesis, "metadata": {"framework": "mrkl", "modules": len(modules)}}
...
Check: entry points, failing tests, logs, config. What's the current state?""",
        ...
        # Check if green
        if (
            all(word in verification.lower() for word in ["pass", "green"])
            or "success" in verification.lower()
        ):
            summary = await sampler.request_sample(
                f"Summarize:\n\n{patch}\n\nChanges made + remaining risks.", temperature=0.4
            )
            return {
