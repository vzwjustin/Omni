...
class DocumentMetadata:
    ...
    framework_name: Optional[str] = None  # e.g., "active_inference"
    framework_category: Optional[str] = None  # e.g., "iterative"
    module_path: Optional[str] = None  # e.g., "app.nodes.iterative"
...
class CodeAnalyzer:
    ...
    @staticmethod
    def extract_structure(code: str, file_path: str) -> List[Tuple[str, DocumentMetadata, int, int]]:
        """
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                ...
                if chunk_content:
                    chunks.append((chunk_content, metadata, node.lineno, node.end_lineno or node.lineno))
    ...
    @staticmethod
    def _extract_function(node: ast.FunctionDef, code: str, file_path: str) -> Tuple[str, DocumentMetadata]:
    ...
    @staticmethod
    def _extract_class(node: ast.ClassDef, code: str, file_path: str) -> Tuple[str, DocumentMetadata]:
...
class MarkdownAnalyzer:
        ...
        for i, part in enumerate(sections):
            if re.match(r'^#{1,6}\s+', part):
                ...
                if current_section.strip():
                    chunks.append(MarkdownAnalyzer._create_section_chunk(
    ...
    @staticmethod
    def _create_section_chunk(header: str, content: str, file_path: str, index: int) -> Tuple[str, DocumentMetadata]:
...
def categorize_file(file_path: Path) -> FileCategory:
    ...
    elif 'nodes' in path_str and any(cat in path_str for cat in ['strategy', 'search', 'iterative', 'code', 'context', 'fast']):
...
def extract_framework_info(file_path: Path) -> Tuple[Optional[str], Optional[str]]:
