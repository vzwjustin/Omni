...
class TestToolRegistrationLeanMode:
    ...
    async def test_lean_mode_tool_count(self, mock_settings_lean):
        ...
        with (
            patch("server.main.get_settings", return_value=mock_settings_lean),
            patch("server.main.LEAN_MODE", True),
        ):
            from server.main import create_server
...
class TestToolRegistrationFullMode:
    ...
    async def test_full_mode_tool_count(self, mock_settings_full):
        ...
        with (
            patch("server.main.get_settings", return_value=mock_settings_full),
            patch("server.main.LEAN_MODE", False),
        ):
...
class TestCallToolRouting:
    ...
    async def test_route_to_reason_handler(
        ...
        with (
            patch("server.main.get_settings", return_value=mock_settings_lean),
            patch("server.main.LEAN_MODE", True),
            patch("server.main.get_rate_limiter", return_value=mock_rate_limiter),
            patch("server.main.get_collection_manager", return_value=mock_collection_manager),
            patch("server.main.handle_reason") as mock_handler,
        ):
            ...
            server = create_server()
            await server.call_tool_handler("reason", {"query": "test"})
    ...
    async def test_route_to_health_handler(
        ...
        with (
            patch("server.main.get_settings", return_value=mock_settings_lean),
            patch("server.main.LEAN_MODE", True),
            patch("app.core.rate_limiter.get_rate_limiter", AsyncMock(return_value=mock_rate_limiter)),
            patch("server.main.get_collection_manager", return_value=mock_collection_manager),
            patch("server.main.handle_health") as mock_handler,
        ):
    ...
    async def test_route_to_think_framework(
        ...
        with (
            ...
            patch("app.core.rate_limiter.get_rate_limiter", AsyncMock(return_value=mock_rate_limiter)),
            patch("server.main.get_collection_manager", return_value=mock_collection_manager),
            patch("server.main.handle_think_framework") as mock_handler,
        ):
            ...
            server = create_server()
            await server.call_tool_handler(
...
class TestRateLimiting:
    ...
    async def test_rate_limit_rejected(self, mock_settings_lean, mock_collection_manager):
        ...
        with (
            patch("server.main.get_settings", return_value=mock_settings_lean),
            patch("server.main.LEAN_MODE", True),
            patch("app.core.rate_limiter.get_rate_limiter", AsyncMock(return_value=rate_limiter)),
            patch("server.main.get_collection_manager", return_value=mock_collection_manager),
        ):
            from server.main import create_server
...
class TestCorrelationId:
    ...
    async def test_correlation_id_set_and_cleared(
        ...
        with (
            ...
            patch("app.core.rate_limiter.get_rate_limiter", AsyncMock(return_value=mock_rate_limiter)),
            patch("server.main.get_collection_manager", return_value=mock_collection_manager),
            patch("server.main.set_correlation_id") as mock_set,
            patch("server.main.clear_correlation_id") as mock_clear,
            patch("server.main.handle_health") as mock_handler,
        ):
...
class TestContextUtilityTools:
    ...
    async def test_count_tokens_handler(
        ...
        with (
            ...
            patch("app.core.rate_limiter.get_rate_limiter", AsyncMock(return_value=mock_rate_limiter)),
            patch("server.main.get_collection_manager", return_value=mock_collection_manager),
            patch("server.main.handle_count_tokens") as mock_handler,
        ):
    ...
    async def test_compress_content_handler(
        ...
        with (
            ...
            patch("app.core.rate_limiter.get_rate_limiter", AsyncMock(return_value=mock_rate_limiter)),
            patch("server.main.get_collection_manager", return_value=mock_collection_manager),
            patch("server.main.handle_compress_content") as mock_handler,
        ):
            mock_handler.return_value = [TextContent(
                type="text",
    ...
    async def test_detect_truncation_handler(
        ...
        with (
            ...
            patch("app.core.rate_limiter.get_rate_limiter", AsyncMock(return_value=mock_rate_limiter)),
            patch("server.main.get_collection_manager", return_value=mock_collection_manager),
            patch("server.main.handle_detect_truncation") as mock_handler,
        ):
            mock_handler.return_value = [TextContent(
                type="text",
    ...
    async def test_manage_claude_md_handler(
        ...
        with (
            ...
            patch("app.core.rate_limiter.get_rate_limiter", AsyncMock(return_value=mock_rate_limiter)),
            patch("server.main.get_collection_manager", return_value=mock_collection_manager),
            patch("server.main.handle_manage_claude_md") as mock_handler,
        ):
            mock_handler.return_value = [TextContent(
                type="text",
                text='{"presets": {}}'
...
class TestPrepareContextTools:
    ...
    async def test_prepare_context_handler(
        ...
        with (
            ...
            patch("app.core.rate_limiter.get_rate_limiter", AsyncMock(return_value=mock_rate_limiter)),
            patch("server.main.get_collection_manager", return_value=mock_collection_manager),
            patch("server.main.handle_prepare_context") as mock_handler,
        ):
            mock_handler.return_value = [TextContent(
                type="text",
    ...
    async def test_prepare_context_streaming_handler(
        ...
        with (
            ...
            patch("app.core.rate_limiter.get_rate_limiter", AsyncMock(return_value=mock_rate_limiter)),
            patch("server.main.get_collection_manager", return_value=mock_collection_manager),
            patch("server.main.handle_prepare_context_streaming") as mock_handler,
        ):
            mock_handler.return_value = [TextContent(
                type="text",
    ...
    async def test_context_cache_status_handler(
        ...
        with (
            ...
            patch("app.core.rate_limiter.get_rate_limiter", AsyncMock(return_value=mock_rate_limiter)),
            patch("server.main.get_collection_manager", return_value=mock_collection_manager),
            patch("server.main.handle_context_cache_status") as mock_handler,
        ):
            mock_handler.return_value = [TextContent(
                type="text",
...
class TestExecuteCodeTool:
    ...
    async def test_execute_code_handler(
        ...
        with (
            ...
            patch("app.core.rate_limiter.get_rate_limiter", AsyncMock(return_value=mock_rate_limiter)),
            patch("server.main.get_collection_manager", return_value=mock_collection_manager),
            patch("server.main.handle_execute_code") as mock_handler,
        ):
            mock_handler.return_value = [TextContent(
                type="text",
...
class TestFullModeSpecificTools:
    ...
    async def test_list_frameworks_routing(
        ...
        with (
            ...
            patch("app.core.rate_limiter.get_rate_limiter", AsyncMock(return_value=mock_rate_limiter)),
            patch("server.main.get_collection_manager", return_value=mock_collection_manager),
            patch("server.main.handle_list_frameworks") as mock_handler,
        ):
            mock_handler.return_value = [TextContent(
                type="text",
                text="# Frameworks\n\n## STRATEGY\n..."
            ...
            server = create_server()
            await server.call_tool_handler("list_frameworks", {})
    ...
    async def test_recommend_routing(
        ...
        with (
            ...
            patch("app.core.rate_limiter.get_rate_limiter", AsyncMock(return_value=mock_rate_limiter)),
            patch("server.main.get_collection_manager", return_value=mock_collection_manager),
            patch("server.main.handle_recommend") as mock_handler,
        ):
            mock_handler.return_value = [TextContent(
                type="text",
    ...
    async def test_rag_search_tools_routing(
        ...
        for tool_name, args in rag_tools:
            ...
            with (
                ...
                patch("app.core.rate_limiter.get_rate_limiter", AsyncMock(return_value=mock_rate_limiter)),
                patch("server.main.get_collection_manager", return_value=mock_collection_manager),
                patch(f"server.main.{handler_name}") as mock_handler,
            ):
                mock_handler.return_value = [TextContent(
                    type="text",
                    text="Search results..."
                ...
                server = create_server()
                await server.call_tool_handler(tool_name, args)
...
class TestMemoryTools:
    ...
    async def test_get_context_routing(
        ...
        with (
            ...
            patch("app.core.rate_limiter.get_rate_limiter", AsyncMock(return_value=mock_rate_limiter)),
            patch("server.main.get_collection_manager", return_value=mock_collection_manager),
            patch("server.main.handle_get_context") as mock_handler,
        ):
            mock_handler.return_value = [TextContent(
                type="text",
    ...
    async def test_save_context_routing(
        ...
        with (
            ...
            patch("app.core.rate_limiter.get_rate_limiter", AsyncMock(return_value=mock_rate_limiter)),
            patch("server.main.get_collection_manager", return_value=mock_collection_manager),
            patch("server.main.handle_save_context") as mock_handler,
        ):
            mock_handler.return_value = [TextContent(
                type="text",
